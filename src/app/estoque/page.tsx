...
import { products, stockCountByStore, stores } from "@/lib/mock-data";
  ...
  return (
    <div className="rounded-xl border border-neutral-200 bg-white overflow-x-auto">
      <table className="w-full text-sm">
        ...
        <tbody>
          {products.map((p) => {
            const abaixoMinimo = p.estoqueCentral < p.estoqueMin;
            return (
              <tr key={p.id} className="border-b border-neutral-50 last:border-0">
                ...
                <td className="px-4 py-3 text-right text-neutral-900">
                  {p.estoqueCentral.toLocaleString("pt-BR")} {p.unidadeConsumo}
                </td>
                <td className="px-4 py-3 text-right text-neutral-500">
                  {p.estoqueMin.toLocaleString("pt-BR")} {p.unidadeConsumo}
                </td>
                <td className="px-4 py-3 text-right text-neutral-500">
                  R$ {p.custoMedio.toFixed(3)} / {p.unidadeConsumo}
                </td>
