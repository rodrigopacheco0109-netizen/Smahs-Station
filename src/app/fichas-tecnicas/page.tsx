...
import { getFichasTecnicas, type FichaTecnica } from "@/server/actions/fichas-tecnicas";
...
export default async function FichasTecnicasPage() {
  ...
  const porLoja = new Map<string, FichaTecnica[]>();
  for (const ficha of fichas) {
    const lista = porLoja.get(ficha.storeNome) ?? [];
    lista.push(ficha);
    porLoja.set(ficha.storeNome, lista);
  }
  ...
  return (
    <div className="space-y-10">
      ...
      {[...porLoja.entries()].map(([storeNome, itens]) => (
        <div key={storeNome} className="space-y-4">
          <h2 className="text-sm font-semibold text-neutral-900">{storeNome}</h2>
